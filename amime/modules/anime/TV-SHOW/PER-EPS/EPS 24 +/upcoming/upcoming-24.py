...
@Amime.on_callback_query(filters.regex(r"^tvshow_24plus_upcoming anime (?P<page>\d+)"))
    ...
    async with httpx.AsyncClient(http2=True) as client:
            ...
            json=dict(
                ...
                query($per_page: Int) {
                    Page(page: 1, perPage: $per_page) {
                        media(type: ANIME, format: TV, sort: POPULARITY_DESC, status: NOT_YET_RELEASED, episodes_greater: 20, episodes_lesser: 30) {
                            id
                            title {
                                romaji
                                english
        ...
        if data["data"]:
            ...
            layout = Pagination(
                ...
                page_data=lambda pg: f"tvshow_24plus_upcoming anime {pg}",
            )
