...
@Amime.on_callback_query(filters.regex(r"^tvshow_24-_top anime (?P<page>\d+)"))
    ...
    async with httpx.AsyncClient(http2=True) as client:
            ...
            json=dict(
                ...
                query($per_page: Int) {
                    Page(page: 1, perPage: $per_page) {
                        media(type: ANIME, format: TV, sort: SCORE_DESC, status: FINISHED, episodes_greater: 0, episodes_lesser: 23) {
                            id
                            title {
                                romaji
                                english
        ...
        if data["data"]:
            ...
            layout = Pagination(
                ...
                page_data=lambda pg: f"tvshow_24-_top anime {pg}",
            )
            ...
            if len(lines) > 0:
                keyboard += lines
    keyboard.append([(lang.Next, "tvshow_24-_top1 anime 1")])
