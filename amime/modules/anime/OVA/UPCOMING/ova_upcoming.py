...
@Amime.on_callback_query(filters.regex(r"^ova_upcoming anime (?P<page>\d+)"))
    ...
    async with httpx.AsyncClient(http2=True) as client:
            ...
            json=dict(
                ...
                query($per_page: Int) {
                    Page(page: 1, perPage: $per_page) {
                        media(type: ANIME, format: OVA, sort: SCORE_DESC, status: NOT_YET_RELEASED) {
                            id
                            title {
                                romaji
                                english
        ...
        if data["data"]:
            ...
            layout = Pagination(
                ...
                page_data=lambda pg: f"ova_upcoming anime {pg}",
            )
