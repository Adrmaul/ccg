...
@Amime.on_callback_query(filters.regex(r"^top_movie anime (?P<page>\d+)"))
async def anime_suggestions(bot: Amime, callback: CallbackQuery):
    ...
    async with httpx.AsyncClient(http2=True) as client:
        ...
        if data["data"]:
            ...
            layout = Pagination(
                suggestions,
                item_data=lambda i, pg: (f"menu {i.id}", (lang.search1_button, f"!a {result.title.romaji}", "switch_inline_query_current_chat")),
            ...
            if len(lines) > 0:
                keyboard += lines
    keyboard.append([(lang.Next, "top_movie1 anime 1")])
