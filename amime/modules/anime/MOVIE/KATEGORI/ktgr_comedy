...
@Amime.on_callback_query(filters.regex(r"^movie_comedy anime (?P<page>\d+)"))
    ...
    async with httpx.AsyncClient(http2=True) as client:
            ...
            json=dict(
                ...
                query($page: Int, $perPage: Int) {
                    Page(page: $page, perPage: $perPage) {
                        media(type: ANIME, format: MOVIE, sort: TRENDING_DESC, genre: "comedy") {
                            id
                            title {
                                romaji
                                english
        ...
        if data["data"]:
            ...
            layout = Pagination(
                ...
                page_data=lambda pg: f"movie_comedy anime {pg}",
            )
            ...
            if len(lines) > 0:
                keyboard += lines
    keyboard.append([(lang.Next, "movie_comedy anime 1")])
